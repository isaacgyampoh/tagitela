...
const IDLE_PANELS = [
  ...
  { k: 'location', title: 'Find us', sub: 'Sempe Mensah St, Accra' },
  { k: 'contact', title: 'Talk to us', sub: '054 073 2878  ·  057 500 4311' },
]
...
export default function CustomerDisplay() {
  ...
  if (s.status === 'paid') {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#0f172a] text-white px-6 relative overflow-hidden">
        ...
        <p className="text-white/30 mt-12 text-xs tracking-[0.2em] uppercase">TAGITELA &middot; Accra</p>
      </div>
    )
  }
  ...
  return (
    <div className="fixed inset-0 bg-white overflow-hidden">
      {/* Header — fixed at top */}
      <header className="fixed top-0 left-0 right-0 z-20 flex items-center gap-3 px-8 py-5 bg-[#0f172a] text-white">
        <LogoFlat height={22} color="#ffffff" accent="#9a9da3" tagline={false} />
        {s.status === 'paying' && <span className="ml-auto text-sm font-semibold bg-white text-[#0f172a] px-4 py-1.5 rounded-full">Complete payment on terminal</span>}
      </header>
      ...
      {empty ? (
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6 bg-white overflow-hidden">
          ...
          <div className="relative idle-breath">
            <Logo height={120} color="#0f172a" accent="#9a9da3" tagline={true} className="mb-10" />
          </div>
          ...
          <div key={idleIdx} className="idle-rise relative" style={{ minHeight: '120px' }}>
            <h1 className="text-4xl md:text-5xl font-bold text-[#0f172a] mb-3 tracking-tight">{IDLE_PANELS[idleIdx].title}</h1>
          ...
          <div className="absolute bottom-12 flex gap-2 idle-drift">
            {IDLE_PANELS.map((_, i) => (
              <span key={i} className={`h-1.5 rounded-full transition-all duration-500 ${i === idleIdx ? 'w-6 bg-[#0f172a]' : 'w-1.5 bg-[#d8d9d7]'}`} />
            ))}
          </div>
        </div>
      ) : (
        <>
          ...
          <div className="fixed left-0 right-0 bottom-0 lg:left-auto lg:top-0 lg:w-96 z-20 bg-[#0f172a] text-white px-8 py-6 lg:py-8 lg:flex lg:flex-col lg:justify-end shadow-[0_-8px_30px_rgba(0,0,0,0.25)] lg:shadow-none">
