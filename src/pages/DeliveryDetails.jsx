...
export default function DeliveryDetails() {
  ...
  if (order === null) return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 text-center">
      <Logo height={64} color="#0f172a" accent="#9a9da3" className="mb-6" />
  ...
  if (done) return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 text-center">
      <div className="w-16 h-16 rounded-full bg-[#2563eb] flex items-center justify-center mb-5">
    ...
    <div className="min-h-screen py-8 px-5">
      <div className="max-w-md mx-auto">
        <div className="text-center mb-6">
          <Logo height={56} color="#0f172a" accent="#9a9da3" className="mb-4 mx-auto" />
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-5 space-y-4">
          <div>
            ...
            <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Your full name" className="w-full h-12 px-4 bg-gray-50 border-2 border-gray-200 rounded-xl text-base focus:outline-none focus:border-[#2563eb]" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1.5">Phone Number *</label>
            <input value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} inputMode="tel" placeholder="024 000 0000" className="w-full h-12 px-4 bg-gray-50 border-2 border-gray-200 rounded-xl text-base focus:outline-none focus:border-[#2563eb]" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1.5">Delivery Address *</label>
            <textarea value={form.address} onChange={e => setForm({ ...form, address: e.target.value })} rows={3} placeholder="House number, street, area, town/city" className="w-full px-4 py-3 bg-gray-50 border-2 border-gray-200 rounded-xl text-base focus:outline-none focus:border-[#2563eb] resize-none" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1.5">Nearest Landmark</label>
            <input value={form.landmark} onChange={e => setForm({ ...form, landmark: e.target.value })} placeholder="e.g. near the blue church" className="w-full h-12 px-4 bg-gray-50 border-2 border-gray-200 rounded-xl text-base focus:outline-none focus:border-[#2563eb]" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1.5">Delivery Notes</label>
            <input value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} placeholder="Anything we should know" className="w-full h-12 px-4 bg-gray-50 border-2 border-gray-200 rounded-xl text-base focus:outline-none focus:border-[#2563eb]" />
          </div>

          <button onClick={save} disabled={saving || !form.name.trim() || !form.address.trim()} className="w-full h-13 bg-[#2563eb] text-white rounded-xl font-bold text-base disabled:opacity-40 mt-2">
